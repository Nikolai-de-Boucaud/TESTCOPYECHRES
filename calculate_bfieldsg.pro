;MVZ got rid of inclusion of a file  just taken from calculate_bfields 11/17/06
...
pro calculate_bfieldsg,bp,br,bt,bz,g

;if n_elements(a.shot) eq 0 or n_elements(g.time) eq 0 then return
;if  n_elements(g.time) eq 0 then return

;if a.ishot le 0 or g.time le 0 then return
;if a.shot le 0 or g.time le 0 then return
