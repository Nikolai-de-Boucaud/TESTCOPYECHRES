...
bad_time:				; error - usually '****'
  ...
  if (gtime ne gtimeFile and (long(gtime) eq gtime)) then begin
    ...
    gtime = gtimeFile   		; for these cases, data in the geqdsk struct will be returned from the 
					; file, so ensure that g.time gets set to gtimeFile so that the return
					; struct is consistant.
  endif
