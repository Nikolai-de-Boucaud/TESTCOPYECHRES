...
; PURPOSE: 
;
;	Retrieves Measurements data from MDSplus and writes into a 
; 	netCDF 'M' file.
;
; CATEGORY: 
;
...
; CALLING SEQUENCE: 
;
;	a = WRITEM(arg1, arg2   [,RUNID=runid] [,INFO=info] 
;				[,FILE=file] [,OVERWRITE=overwrite]
;                               [,EXACT_TIME=exact_time] [,VERBOSE=verbose] 
;                               [,DEBUG=debug] [,STATUS=status] )
;
;
; INPUT PARAMETERS: 
;
;	arg1:  	A long integer specifying the shot number.
;
...
;              to find the EFIT.
;	
;	FILE:	Specify the filename. Defaults to mssssss.ttttt
;
;	OVERWRITE: Overwrite without prompt if file exists. Defaults to prompt.
;
...
;
;	2002-02-07 create by Q. Peng based on readm.pro
;
...
; 	2012.04.09 SMF - Add support for microsecond file names (m######.#####_###)
;-	
...
PRO writem_file, m, file=file, overwrite=overwrite

  IF N_Elements(file) EQ 0 THEN BEGIN
   sshot = strtrim(m.shot,2);
   IF strlen(sshot) GT 6 THEN BEGIN
      sshot = strmid(sshot,0,6)
      Message,'Using '+sshot+' for shot number',/Info
   ENDIF
   micro = (strtok(string(m.time,format='(f10.3)'),'.',/extract))[1]
   file = 'm'+string(sshot,format='(i6.6)')+'.'+string(m.time,format='(i5.5)')
   if ( long(micro) ne 0 ) then file=file+'_'+micro
  ENDIF

  IF (file_search(file))[0] NE '' AND NOT Keyword_Set(overwrite) THEN BEGIN
     s = ''
     read,s,prompt=file+' exists, overwrite (Y/N):'
     IF StrUpcase(s) NE 'Y' THEN Return
  ENDIF
...
; def dimensions
  ntime = ncdf_dimdef(fid,'dim_time',/UNLIMITED)
  scalar = ncdf_dimdef(fid,'dim_scalar',1)

  dims = ['nstark','nsilop','magpri','nfcoil','nesum' ,'npress','npcurn','nitera']
  vars = ['cmgam', 'csilop','cmpr2', 'fccurt','eccurt','pressr','xrsp'  ,'cchisq']
  FOR i=0, N_Elements(dims)-1 DO okay = execute($
     dims[i]+"= ncdf_dimdef(fid,'dim_"+dims[i]+"',N_Elements(m."+vars[i]+"))")

; define variables
  shot = ncdf_vardef(fid,'shot',scalar,/long)
  time = ncdf_vardef(fid,'time',ntime)
  tangam = ncdf_vardef(fid,'tangam',[nstark,ntime])
  cmgam = ncdf_vardef(fid,'cmgam',[nstark,ntime])
  siggam = ncdf_vardef(fid,'siggam',[nstark,ntime])
  rrgam = ncdf_vardef(fid,'rrgam',[nstark,ntime])
  zzgam = ncdf_vardef(fid,'zzgam',[nstark,ntime])
  a1gam = ncdf_vardef(fid,'a1gam',[nstark,ntime])
  a2gam = ncdf_vardef(fid,'a2gam',[nstark,ntime])
  a3gam = ncdf_vardef(fid,'a3gam',[nstark,ntime])
  a4gam = ncdf_vardef(fid,'a4gam',[nstark,ntime])
  a5gam = ncdf_vardef(fid,'a5gam',[nstark,ntime])
  a6gam = ncdf_vardef(fid,'a6gam',[nstark,ntime])
  fwtgam = ncdf_vardef(fid,'fwtgam',[nstark,ntime])
  msebkp = ncdf_vardef(fid,'msebkp',ntime)
  silopt = ncdf_vardef(fid,'silopt',[nsilop,ntime])
  csilop = ncdf_vardef(fid,'csilop',[nsilop,ntime])
  fwtsi = ncdf_vardef(fid,'fwtsi',[nsilop,ntime])
  expmpi = ncdf_vardef(fid,'expmpi',[magpri,ntime])
  fwtmp2 = ncdf_vardef(fid,'fwtmp2',[magpri,ntime])
  cmpr2 = ncdf_vardef(fid,'cmpr2',[magpri,ntime])
  fccurt = ncdf_vardef(fid,'fccurt',[nfcoil,ntime])
  ccbrsp = ncdf_vardef(fid,'ccbrsp',[nfcoil,ntime])
  fwtfc = ncdf_vardef(fid,'fwtfc',[nfcoil,ntime])
  eccurt = ncdf_vardef(fid,'eccurt',[nesum,ntime])
  cecurr = ncdf_vardef(fid,'cecurr',[nesum,ntime])
  fwtec = ncdf_vardef(fid,'fwtec',[nesum,ntime])
  diamag = ncdf_vardef(fid,'diamag',ntime)
  cdflux = ncdf_vardef(fid,'cdflux',ntime)
  sigdia = ncdf_vardef(fid,'sigdia',ntime)
  plasma = ncdf_vardef(fid,'plasma',ntime)
  cpasma = ncdf_vardef(fid,'cpasma',ntime)
  darea = ncdf_vardef(fid,'darea',ntime)
  xrsp = ncdf_vardef(fid,'xrsp',[npcurn,ntime])
  pressr = ncdf_vardef(fid,'pressr',[npress,ntime])
  cpress = ncdf_vardef(fid,'cpress',[npress,ntime])
  rpress = ncdf_vardef(fid,'rpress',[npress,ntime])
  zpress = ncdf_vardef(fid,'zpress',[npress,ntime])
  sigpre = ncdf_vardef(fid,'sigpre',[npress,ntime])
  saipre = ncdf_vardef(fid,'saipre',[npress,ntime])
  chigam = ncdf_vardef(fid,'chigam',[nstark,ntime])
  saimpi = ncdf_vardef(fid,'saimpi',[magpri,ntime])
  saisil = ncdf_vardef(fid,'saisil',[nsilop,ntime])
  czmaxi = ncdf_vardef(fid,'czmaxi',[nitera,ntime])
  cchisq = ncdf_vardef(fid,'cchisq',[nitera,ntime])
  cerror = ncdf_vardef(fid,'cerror',[nitera,ntime])

  ncdf_attput,fid,/Global,'title','EFIT measurement file generated from MDSplus'

  ncdf_control,fid,/endef	; enter data mode

; write variables
  name = Tag_Names(m)
  vars = name[Where(name NE 'ERROR' AND name NE 'SOURCE' $
	        AND name NE 'MSG' AND name NE 'MTIME')]
  FOR i=0, N_Elements(vars)-1 DO $
  okay = execute("ncdf_varput,fid,"+vars[i]+",m."+vars[i])
...
PRO writem, arg1, arg2, convert=convert, mode=mode, runid=runid, info=info,$ 
			    source=source, $
			    exact_time=exact_time, time_range=time_range, $
			    verbose=verbose, debug=debug, status=status, $
			    _Extra=extra

  forward_function readm
  ...
  m = readm(arg1, arg2, mode='MDSPLUS', runid=runid, status=status, _EXTRA = e)

  IF status THEN writem_file,m,_Extra=extra $
  ELSE Message,'failed reading M from MDSplus: '+strtrim(status,2),/Informational
