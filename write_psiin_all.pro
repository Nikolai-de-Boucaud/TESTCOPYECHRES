...
; write_psiin_all.pro
; same as write_psiin, but fluxfun_ech(g) is an argument

pro write_psiin_all, g, f, outfile=outfile, $
	ledge=ledge, gafsep=gafsep, lcentr=lcentr, status=status
; g = gstruct is structure from g-file
; f = ffstruct is structure from fluxfun_ech(g)
; outfile keyword is name of output file if different than 'psiin'
; ledge keyword is radial resolution if different than 201
